...
class Kpi_profil extends _adminController
{
    ...
    protected  function init()
    {
        ...
        // $this->load->model("Risk_kegiatanModel", "modelKegiatan");
    ...
    public function Edit($page = null)
    {
        ...
        if ($page)
            $this->data['periodearr'] = $this->conn->GetList("select id_contents as idkey, tgl_mulai_aktif val from contents where page = " . $this->conn->escape($page));
...
*/
    ...
    {
        if (!$page) {
            redirect('panelbackend/kpi_profil/detail/kpi_visi_misi_corevalues');
            die();
        }
    }
    ...
    public function Add($page = null)
    {
        ...
        if ($this->post['act'] === 'save') {
            ...
            if ($return) {
                $last_id = $this->conn->GetOne("select max(id_contents) from contents where page = " . $this->conn->escape($page));
                // $return = $this->model->Update($record, "$this->pk = " . $this->conn->qstr($id));
        ...
        $this->data['pk'] = 'page';
        switch ($page) {
            case 'kpi_visi_misi_corevalues':
                $this->data['page_title'] = "VISI, MISI, CORE VALUES, & ARAH KEBIJAKAN";
                break;
            case 'sasaran_target_strategi_kpi_perusahaan':
                $this->data['page_title'] = "SASARAN, TARGET, STRATEGI, KPI PERUSAHAAN";
                break;
            case 'kpi_perusahaan':
                $this->data['page_title'] = "KPI PERUSAHAAN";
                break;
            case 'kpi_unit_kerja':
                $this->data['page_title'] = "KPI UNIT KERJA";
                break;
            default:
    ...
    public function Detail($page = null)
    {
        ...
        if ($_SESSION[SESSION_APP][$this->page_ctrl]['periode']) {
            $cek = $this->conn->GetOne("select page from contents where id_contents = " . $this->conn->escape($_SESSION[SESSION_APP][$this->page_ctrl]['periode']));
            if ($cek == $page)
                $periode = $_SESSION[SESSION_APP][$this->page_ctrl]['periode'];
            else
                $periode = array_key_last($this->data['periodearr']);
...
*/
    ...
    protected function _afterDetail($id, $page = null)
    {
		...
		function goAdd(page,base_url){
			window.location = base_url+"panelbackend/kpi_profil/add/"+page;
		}
		</script>';
    }
