...
class Content extends _adminController
{
	...
	public function Add($page = null)
	{
		...
		if ($this->post['act'] === 'save') {
			...
			if ($return['success']) {
				...
				$_SESSION[SESSION_APP][$this->page_ctrl]['periode'][$page] = $id;
		...
		$this->data['pk'] = 'page';
		$this->data['page_title'] =  "Bagan Struktur Organisasi";
	...
	public function Edit($page = null)
	{
		...
		if ($this->post['act'] == 'set_periode') {
			$_SESSION[SESSION_APP][$this->page_ctrl]['periode'][$page] = $this->post['periode'];
			redirect(current_url());
		}

		// dpr(array_key_last($this->data['periodearr']),1);
		$periode = array_key_last($this->data['periodearr']);
		if ($_SESSION[SESSION_APP][$this->page_ctrl]['periode'][$page])
			$periode = $_SESSION[SESSION_APP][$this->page_ctrl]['periode'][$page];
...
*/
	...
	protected function _afterDetail($id, $page = null)
	{
		...
		$this->data['page_title'] =  "Bagan Struktur Organisasi";
		if ($page == 'stakeholder')
			$this->data['page_title'] =  "Stakeholder";
		if ($this->modelfile) {
		...
		$this->data['page_title'] .= UI::createSelect("periode", $this->data['periodearr'], $id, true, '', "style='width:230px; margin-left:10px;' onchange='goSubmit(\"set_periode\")'");
		...
		<script>
		function goAdd(id,base_url){
			window.location = base_url+"panelbackend/content/add/"+id;
		}
		</script>';
	}
