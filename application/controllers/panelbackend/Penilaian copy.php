...
class Penilaian extends _adminController
{
	...
	private function _generate($tahun, $id_kategori)
	{
		...
		if (!$this->data['id_penilaian_session']) {
			$tgl = $tahun . "-01-01";

			$record = [];
			$record['nama'] = $this->data['page_title'] . " " . $tahun;
			$record['tgl'] = $tgl;
			$record['page_ctrl'] = $this->page_ctrl;
			$this->conn->goInsert("penilaian_session", $record);
			$this->data['id_penilaian_session'] = $this->conn->GetOne("select 
			id_penilaian_session 
			from penilaian_session 
			where date_format(tgl,'%Y-%m-%d') = " . $this->conn->escape($tgl) . " 
		...
		$arearr = $this->mkriteria->get_kriteria($id_kategori, $tahun);
