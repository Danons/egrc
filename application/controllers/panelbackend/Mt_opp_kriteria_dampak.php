...
class Mt_opp_kriteria_dampak extends _adminController
{
	...
	{
		parent::init();
		$this->viewlist = "panelbackend/mt_opp_kriteria_dampaklist";
		$this->viewdetail = "panelbackend/mt_opp_kriteria_dampakdetail";
		...
		$this->load->model("Mt_opp_kriteria_dampakModel", "model");
	...
	private function _delSertKeterangan($id)
	{
		...
		$return = $this->conn->Execute("update mt_opp_kriteria_dampak_detail deleted_date = new() where id_kriteria_dampak = " . $this->conn->escape($id));
		...
		if (is_array($this->post['keterangan'])) {
			foreach ($this->post['keterangan'] as $idkey => $value) {
				if ($return) {
					...
					$sql = $this->conn->InsertSQL("mt_opp_kriteria_dampak_detail", $record);
	...
	{
		$return = $this->model->Execute("update mt_opp_kriteria_dampak_detail set deleted_date = now() where id_kriteria_dampak = " . $this->conn->escape($id));
	...
	protected function _afterDetail($id)
	{
		if (!($this->data['row']['keterangan'])) {
			...
			$keteranganarr = $this->conn->GetArray("select id_dampak, keterangan from mt_opp_kriteria_dampak_detail where deleted_date is null and id_kriteria_dampak = " . $this->conn->escape($id));
