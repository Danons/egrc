...
class Quisioner extends _adminController
{
	...
	protected function init()
	{
		...
		$this->data['jabatanarr'] = ['' => 'Filter Unit...'] + $this->mtjabatan->GetCombo();

		$this->load->model("Public_sys_userModel", "userModel");
		$this->data['userarr'] = ['' => 'Filter User...'] + $this->userModel->GetCombo();
	...
	public function Index($id_kategori = null, $page = 0)
	{
		...
		// dpr($_SESSION[SESSION_APP]['user_id'], 1);

		$this->data['id_jabatan_filter'] = $id_jabatan_filter;
		$this->data['id_user_filter'] = $id_user_filter;
		$this->data['page_title'] .= "&nbsp;&nbsp;";
		if ($this->access_role['view_all'])
			$this->data['page_title'] .= UI::createSelect("id_jabatan_filter", $this->data['jabatanarr'], $id_jabatan_filter, $this->access_role['view_all'], 'form-control', "onchange=\"goSubmit('set_filter')\"");
		$this->data['page_title'] .= UI::createSelect("id_user_filter", $this->data['userarr'], $id_user_filter, $this->access_role['view_all'], 'form-control', "onchange=\"goSubmit('set_filter')\"");
		$this->data['edited'] = true;
		...
		if ($id_jabatan_filter) {
			...
			$arr_param = [
				'limit' => -1,
				'filter' => "id_jabatan = " . $this->conn->escape($id_jabatan_filter) . " and id_penilaian_session = " . $this->conn->escape($id_penilaian_session),
			];
		...
		if ($id_user_filter) {
			...
			$arr_param = [
				'limit' => -1,
				'filter' => "id_user = " . $this->conn->escape($id_user_filter) . " and id_penilaian_session = " . $this->conn->escape($id_penilaian_session),
			];
			$this->data['list'] = $this->penilaianquisioner->SelectGrid($arr_param, "*");
			if ($this->access_role['view_all']) {
		...
		if ($id_jabatan_filter) {
			...
			if ($this->post['act'] == 'delete') {
				...
				$ret = $this->conn->Execute("delete from penilaian_quisioner 
			...
			if ($this->post['act'] == 'save') {
				...
				foreach ($this->data['list']['rows'] as &$r) {
					...
					where id_quisioner = " . $this->conn->escape($r['id_quisioner']) . " 
					and id_jabatan = " . $this->conn->escape($id_jabatan_filter) . " 
	...
	protected function _afterDetail($id)
	{
		if (!$this->data['row']['id_jabatanarr'])
			$this->data['row']['id_jabatanarr'] = $this->conn->GetList("select id_jabatan as val from quisioner_jabatan where id_quisioner = " . $this->conn->escape($id));

		if (!$this->data['row']['id_userarr'])
			$this->data['row']['id_userarr'] = $this->conn->GetList("select id_user as val from quisioner_user where id_quisioner = " . $this->conn->escape($id));

		if (!$this->data['row']['id_kriteriaarr'])
			$this->data['row']['id_kriteriaarr'] = $this->conn->GetList("select id_kriteria as val from quisioner_kriteria where id_quisioner = " . $this->conn->escape($id));

		if (!$this->data['row']['tahunarr'])
			$this->data['row']['tahunarr'] = $this->conn->GetList("select tahun as val from quisioner_tahun where id_quisioner = " . $this->conn->escape($id));

		if (!$this->data['row']['pemeriksaanarr'])
			$this->data['row']['pemeriksaanarr'] = $this->conn->GetList("select id_pemeriksaan as val from quisioner_pemeriksaan where id_quisioner = " . $this->conn->escape($id));
	}
	...
	protected function _afterInsert($id = null)
	{
		...
		if ($ret && $this->post['id_jabatanarr']) {
			$ret = $this->conn->Execute("delete from quisioner_jabatan where id_quisioner = " . $this->conn->escape($id));
			foreach ($this->post['id_jabatanarr'] as $k => $v) {
				...
				// dpr($v, 1);
				$this->data['id_userarr'] = $this->conn->GetList("SELECT psu.user_id as val, psu.user_id as idkey FROM public_sys_user psu LEFT JOIN mt_sdm_jabatan msj ON msj.id_jabatan = psu.id_jabatan WHERE msj.id_jabatan = " . $v);
			}
		}
