...
class Pemeriksaan_detail extends _adminController
{
	...
	{
		parent::init();
		$this->viewlist = "panelbackend/pemeriksaan_detaillist";
		$this->viewdetail = "panelbackend/pemeriksaan_detaildetail";
		$this->viewprintdetail = "panelbackend/pemeriksaan_detaildetailprint";
		$this->template = "panelbackend/main";
		$this->layout = "panelbackend/layout_pemeriksaan";

		if ($this->mode == 'add') {
			$this->data['page_title'] = 'Tambah KAK';
			$this->data['edited'] = true;
		} elseif ($this->mode == 'edit') {
			$this->data['page_title'] = 'Edit KAK';
			$this->data['edited'] = true;
		} elseif ($this->mode == 'detail') {
			$this->data['page_title'] = 'Detail Pemeriksaan';
			$this->data['edited'] = false;
		} else {
			$this->data['page_title'] = 'KAK';
		}

		$this->load->model("Pemeriksaan_detailModel", "model");
		$this->load->model("Pemeriksaan_detail_filesModel", "modelfile");
	...
	public function Index($id_pemeriksaan = null, $page = 0)
	{
		...
		$this->_setFilter(" deleted_date is null and id_pemeriksaan = " . $this->conn->qstr($id_pemeriksaan));
		...
		$this->load->model("Pemeriksaan_anggaran_biayaModel", "modelanggaran");

		$param = array(
			'filter' => "deleted_date is null and id_pemeriksaan = " . $this->conn->qstr($id_pemeriksaan)
		);
		$this->data['listanggaran'] = $this->modelanggaran->SelectGrid(
			$param
		);
	...
	public function Detail($id_pemeriksaan = null, $id = null)
	{
		...
		$this->data['rowsreview'] = $this->conn->GetArray("select * 
		from pemeriksaan_review_supervisi 
		where deleted_date is null and id_pemeriksaan_detail = " . $this->conn->escape($id));

		$this->load->model("Pemeriksaan_temuanModel", "temuan");

		$list = $this->temuan->SelectGrid(array(
			'page' => 0,
			'limit' => -1,
			'order' => '',
			'filter' => 'id_pemeriksaan_detail = ' . $this->conn->escape($id)
		));
		$this->data['rowstemuan'] = $list['rows'];
	...
	protected function _beforeDelete($id = null)
	{
		...
		if ($ret) {
			$ret = $this->conn->Execute("update 
			 pemeriksaan_tim set deleted_date = now()
	...
	public function go_print($id_pemeriksaan = null)
	{
		...
		$this->data['page_title'] = "PROGRAM AUDIT PENDAHULUAN";
