...
class Mt_kriteria extends _adminController
{
	...
	{
		$row = $this->conn->GetRow("select * from mt_kategori where deleted_date is null and id_kategori = " . $this->conn->escape($id_kategori));
	...
	{
		$rows = $this->conn->GetArray("select id_kriteria from mt_kriteria where deleted_date is null and id_kriteria_parent = " . $this->conn->escape($id_kriteria));
		foreach ($rows as $r) {
			$this->_updateCild($r['id_kriteria']);
			$this->conn->Execute("update mt_kriteria set deleted_date = now() where is_aktif = 0 and id_kriteria = " . $this->conn->escape($r['id_kriteria']));
		...
		if (in_array($_FILES['importupload']['type'], $file_arr)) {
			...
			for ($row = 2; $row <= $highestRow; $row++) {
				...
				if ($record['kode']) {
					...
					from mt_kriteria 
					where deleted_date is null and id_kategori = " . $this->conn->escape($id_kategori) . "
