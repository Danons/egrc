...
*/
	...
	protected function _afterDetail($id, $page = null)
	{
		...
		if ($this->access_role['add'] && $this->data['edited'] == false)
			$this->data['addbutton'] = '<button type="button" class="btn btn-sm btn-primary" onclick="goAdd(\'' . $page . '\')">Tambah Baru</button>
		<script>
		function goAdd(id){
			window.location = "http://localhost/tirtaraharja_egrc/panelbackend/content/add/"+id;
		}
		</script>';
	}
