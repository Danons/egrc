        ...
        <div class="row justify-content-center">
            <div id="kpi" name="kpi" class="col-lg-8">
                ...
                if (!$masing2 || $masing2 == '12') {
                    ...
                    echo UI::createFormGroup($from, $rules["jenis_direktorat"], "jenis_direktorat", 'KPI Direktorat', false, 2);
                }
                ?>

                <?php
                if ($row['jenis_direktorat'] == '2' || $masing2 == '1') {
                    $from = UI::createRadio("jenis_realisasi_direktorat", $jenisrealisasiarr, $row['jenis_realisasi_direktorat'], $edited, true);
                    echo UI::createFormGroup($from, $rules["jenis_realisasi_direktorat"], "jenis_realisasi_direktorat", "Perhitungan Direktorat", false, 2);
                ...
                <?php if ($row['jenis_direktorat'] == '1' && $masing2 !== '1') { ?>
                    <?php //if ($row['jenis_direktorat'] == '1') { 
                    ?>
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 ">
                        ...
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <?php $url = $row['jenis_direktorat']; ?>
                            <!-- <a href="<?= site_url("$this->page_ctrl/add/3/$id_kpi/$tahun/3") ?>" class="btn btn-sm btn-primary">Tambah Baru</a> -->
                            <a href="<?= site_url("$this->page_ctrl/add/3/$id_kpi/$tahun/$url") ?>" class="btn btn-sm btn-primary">Tambah Baru</a>
                        </div>
                    </div>
                ...
                <?php if ($masing2 !== "1") { ?>
                    <div style="list-style-type: none;margin: 0;padding: 0;overflow: hidden;" class="mt-5 mb-5">
                        <div style="float: left;">
                            <div class="nx-pr pre" onclick="previos('<?= base_url() . 'panelbackend/kpi_target_kor/add/2/' . $id_kpi . '/' . $tahun . '/0\/' . $row['id_kpi_target'] ?>')"> previous</div>
                        </div>
...
<script>
    ...
    function goEdit(id_kpi, tahun, url, id_kpi_target) {
        // console.log(id_kpi)
        // console.log(tahun)
        // console.log(url)
        // console.log(id_kpi_target)
        // console.log('<?= base_url() . "panelbackend/kpi_target_kor/add/3/" ?>' + id_kpi + '/' + tahun + '/' + url + '/' + id_kpi_target);
        window.location = '<?= base_url() . "panelbackend/kpi_target_kor/add/3/" ?>' + id_kpi + '/' + tahun + '/' + url + '/' + id_kpi_target;
    }
