...
<table style="width: 630px;" id="divmatrix">
  <tr>
    <td style="width: max-content;">
      <table class="tbmatrix" style="width: 500px;">
        ...
        foreach ($rs_kemungkinan as $r_k => $val_k) {
          echo "<td align='center' style='width: 25px;text-align:center;vertical-align:middle;font-weight:bold'>" . spacetext($val_k["nama"]) . "</td>";
          ...
          foreach ($rs_dampak as $r_d => $val_d) {
            ...
            if ($top_inheren[$val_d['id_dampak']][$val_k['id_kemungkinan']] && $rating['i']) {
              foreach ($top_inheren[$val_d['id_dampak']][$val_k['id_kemungkinan']] as $n) {
                ...
                // $d = "<div title='" . $noarr[$n]['nama'] . "' class='$cee bg333'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                if ($nokotak > $maxdot)
                  $div1 .= "<div class='$cee bg333'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n " . $noarr[$n]['nama'] . "</a></div>";
                else
                  $div .= "<div data-bs-toggle='tooltip' title='" . $noarr[$n]['nama'] . "' class='$cee bg333'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                // $div .= $d;
              }
            }
            if ($top_paska_kontrol[$val_d['id_dampak']][$val_k['id_kemungkinan']] && $rating['c']) {
              foreach ($top_paska_kontrol[$val_d['id_dampak']][$val_k['id_kemungkinan']] as $n) {
                ...
                // $d = "<div title='" . $noarr[$n]['nama'] . "' class='$cee bg666'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                if ($nokotak > $maxdot)
                  $div1 .= "<div class='$cee bgfff'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n  " . $noarr[$n]['nama'] . "</a></div>";
                else
                  $div .= "<div data-bs-toggle='tooltip' title='" . $noarr[$n]['nama'] . "' class='$cee bg666'><a href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                // $div .= $d;
              }
            }
            if ($top_actual[$val_d['id_dampak']][$val_k['id_kemungkinan']] && $rating['a']) {
              foreach ($top_actual[$val_d['id_dampak']][$val_k['id_kemungkinan']] as $n) {
                ...
                // $d = "<div title='" . $noarr[$n]['nama'] . "' class='$cee bgfff'><a style='font-weight:bold;' href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                if ($nokotak > $maxdot)
                  $div1 .= "<div class='$cee bgfff'><a style='font-weight:bold;' href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n " . $noarr[$n]['nama'] . "</a></div>";
                else
                  $div .= "<div data-bs-toggle='tooltip'  title='" . $noarr[$n]['nama'] . "' class='$cee bgfff'><a style='font-weight:bold;' href='" . site_url("panelbackend/risk_risiko/detail/" . $noarr[$n]['id_scorecard'] . "/" . $noarr[$n]['id_risiko']) . "' target='_BLANK'>$n</a></div>";
                // $div .= $d;
              }
            }

            echo "<td class='bg-$bg' style='border:1px solid #555;background-color:$bg; padding:1px;$css' height='75px' width='75px' align='center' valign='middle'><div style='position:relative;height:75px;width:75px; vertical-align:middle; text-align:center;padding:0px 0px;'>$tingkat_risiko
                  <div style='position:absolute;top:0px;right:0px;'>";
            echo $div;
            if ($div1) {
              // dpr($div1, 1);
              // echo '<div style="background-color:blue;border:1px solid bluecursor: pointer;" rel="tooltip" title="' . $div1 . '" data-html="true" data-placement="right" class="light-tooltip more' . $iii . '" data-container=".more' . $iii . '">+</div>';
              echo '<div style="background-color:blue;color:#fff !important;cursor: pointer;" 
              data-bs-toggle="tooltip" 
              data-bs-title="' . $div1 . '" 
              data-bs-html="true" 
              data-bs-placement="right" 
              data-bs-container=".more' . $iii . '" 
              class="dot zoom-loop light-tooltip more' . $iii . '">+</div>';
        ...
        echo "<tr>";
        foreach ($rs_dampak as $r_d => $val_d) {
          echo "<td style='font-weight:bold;text-align:center;vertical-align:middle'>" . spacetext($val_d["nama"]) . "</td>";
        }
        echo "</tr>";
        echo "<tr>";
    ...
    <td style="vertical-align: top;">
      <table style="margin-left:10px; margin-right:0px; background:#fff; width:120px;">
        <tbody>
          <tr>
            <td colspan="2" style="padding-bottom:10px;font-weight:500">Tingkat Risiko</td>
          </tr>
          <?php
          rsort($tingkatrisikoarr);
          ...
          <tr>
            <td colspan="2" style="padding-bottom:10px;font-weight:500">Level Risiko</td>
          </tr>
          <?php if ($rating['i']) { ?>
            <tr>
              <td style="width: max-content; vertical-align: top; padding-bottom:10px;">
              ...
              <td style="font-size: 12px; padding-bottom:10px;">Inheren Risk</td>
            </tr>
          ...
          if ($rating['c']) { ?>
            <tr>
              ...
              <td style="font-size: 12px; padding-bottom:10px;">Residual Saat Ini</td>
            </tr>
          ...
          if ($rating['a']) { ?>
            <tr>
              ...
              <td style="font-size: 12px; padding-bottom:10px;">Residual Setelah Evaluasi</td>
            </tr>
          <?php } ?>
