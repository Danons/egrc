...
<div class="row mb-3">
    <div class="col-auto me-auto d-flex"><?= UI::createSelect("list_search_filter[id_dit_bid]", $dirarr, $filter_arr["id_dit_bid"], true, 'form-control', "style='width:320px;display:inline;'") ?>
    </div>
...
<table class="table table-hover dataTable treetable table-bordered">
    <thead>
        <tr>
            <th rowspan="2" style="width: 120px;">No</th>
            <th rowspan="2" style="width: 340px;">Indikator KPI</th>
    ...
    <tbody>
        ...
        $no = 0;
        $totalbobotbersama = 0;
        $totalbobotbersamarealisasi = 0;
        foreach ($rows as $row) {
            if ($id_parent && $id_parent <> $row['id_parent']) {
        ?>
                ...
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td style="text-align: right;"><b><?= $id_parent == 'bersama' ? $totalbobotbersama : $totalbobot ?></b></td>
                    <td></td>
                    <td></td>
                    <td style="text-align: right;"><b><?= $id_parent == 'bersama' ? $totalbobotbersamarealisasi : $totalbobotrealisasi ?></b></td>
                    <td></td>
                </tr>
            <?php
                $totalbobot = $totalbobotbersama;
                $totalbobotrealisasi = $totalbobotbersamarealisasi;
                $no = 0;
            }
            ...
            if ($row['id_parent'] == 'bersama') {
                $totalbobotbersama += $row['bobot'];
                $totalbobotbersamarealisasi += (float)$row['realbobot'];
            } else {
                    ...
                    <div class="dropdown" style="display:inline">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenu2">
                            ...
                            <?php if (Access("add", "panelbackend/kpi_target_kor")) { ?>
                                <li><a href="javascript:void(0)" class=" dropdown-item " onclick="goEdit('<?= $row['id_kpi_ori'] ?>','<?= $row['tahun'] ?>','<?= $row['is_bersama'] ?>','<?= $row['id_kpi_target'] ?>')"><i class="bi bi-pencil"></i> Edit</a> </li>
                            <?php } ?>
...
<script>
    ...
    function goEdit(id, tahun, is_bersama, id_kpi_target) {
        if (is_bersama == 1) {
            window.location = '<?= base_url() . "panelbackend/kpi_target_kor/add/3/" ?>' + id + '/' + tahun + '/2/' + id_kpi_target;
        } else {
            window.location = '<?= base_url() . "panelbackend/kpi_target_kor/add/3/" ?>' + id + '/' + tahun + '/1/' + id_kpi_target;
        }
    }

    function goDelete(id) {
        if (confirm("Apakah Anda yakin akan menghapus ?")) {
            window.location = '<?= base_url() . "panelbackend/kpi_target_dir/delete/" ?>' + id;
        }
    }
</script>
