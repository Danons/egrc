    ...
    <tbody>
        ...
        foreach ($list['rows'] as $rows) {
            ...
            foreach ($header as $rows1) {
                $val = $rows[$rows1['name']];
                if ($rows1['name'] == 'nomor_surat' or $rows1['name'] == 'code') {
                    echo "<td><a href='" . ($url = base_url($page_ctrl . "/detail/$rows[$pk]")) . "'>$val</a></td>";
                } elseif ($rows1['name'] == 'periode_pemeriksaan_mulai') {
                    echo "<td>" . Eng2Ind($rows['periode_pemeriksaan_mulai']) . " sd " . Eng2Ind($rows['periode_pemeriksaan_selesai']) . "</td>";
    	...
    	" . UI::showMenuMode('printdetail', $rows[$pk]) . "
    	</td>";
            echo "</tr>";
        }
        if (!count($list['rows'])) {
