...
<div class="row">
  <div class="col-sm-6">
    ...
    echo UI::createFormGroup($from, $rules["tgl_peluang"], "tgl_peluang", "Tgl. Peluang", false, 4, $editedheader1);
    ...
    if (($rowheader1['tgl_close'] or $row['status_peluang'] == 0 or $row['status_peluang'] == 2) && $row['id_peluang']) {
      ...
      echo UI::createFormGroup($from, $rules["tgl_close"], "tgl_close", "Tgl. Close", false, 4, $editedheader1);
    }
    ?>
    ...
    if (strtolower(trim($rowheader['id_unit'])) == '1') {
      $from = UI::createSelectMultiple('id_peluang_unit[]', $peluangunitarr, $row['id_peluang_unit'], $edited, 'select2peluangunit', "
      data-ajax--data-type=\"json\" 
      data-ajax--url=\"" . base_url('panelbackend/ajax/listpeluangunit') . "\" 
      style='width:100%;'");
      echo UI::createFormGroup($from, $rules["id_peluang_unit"], "id_peluang_unit", "Peluang Unit", false, 4, $editedheader1);
    } ?>
    <?php
    // dpr($rowheader1,1);
    ...
    echo UI::createFormGroup($from, $rules["id_sasaran"], "id_sasaran", "Sasaran", false, 4, $editedheader1);

    // $from = UI::createTextArea('sasaran', $rowheader1['sasaran'], '5', '', $editedheader1, $class = 'form-control', "");
    // echo UI::createFormGroup($from, $rules["sasaran"], "sasaran", "Sasaran", false, 4, $editedheader1);
    ?>
    ...
    // echo UI::createFormGroup($from, $rules["id_taksonomi_area"], "id_taksonomi_area", "Taksonomi", false, 4, $editedheader1);

    // if ($is_regulasi) {
    //   $from = UI::createTextArea('regulasi', $rowheader1['regulasi'], '', '', $editedheader1, $class = 'form-control', "");
    //   echo UI::createFormGroup($from, $rules["regulasi"], "regulasi", "Regulasi", false, 4, $editedheader1);
    // }
    ...
    //   echo UI::createFormGroup($from, $rules["id_peluang_parent"], "id_peluang_parent", "Peluang Induk", false, 4, $editedheader1);
    // }
  ...
  <div class="col-sm-6">
    ...
    foreach ($kelayakanarr as $k => $v) {
      $from .= UI::createCheckBox('id_kelayakan[' . $k . ']', $k, $row['id_kelayakan'][$k], $v, $edited) . " <br/>";
    }
    echo UI::createFormGroup($from, $rules["id_kelayakan"], "id_kelayakan", "Studi Kelayakan (Feasibility Study)", false, 4, $editedheader1);

    // $from = UI::createCheckBox('is_kerangka_acuan_kerja', 1, $row['is_kerangka_acuan_kerja'], "Kerangka Acuan Kerja", $edited);
    // echo UI::createFormGroup($from, $rules["is_kerangka_acuan_kerja"], "is_kerangka_acuan_kerja", null, false, 4, $editedheader1);
    // 
    ?>
    ...
    echo UI::createFormGroup($from, $rules["dampak"], "dampak", "Manfaat", false, 4, $editedheader1);
    ?>

    <?php
    $from = UI::createTextBox('anggaran_biaya', $rowheader1['anggaran_biaya'], '', '', $editedheader1, 'form-control rupiah');
    echo UI::createFormGroup($from, $rules["anggaran_biaya"], "anggaran_biaya", "Anggaran Biaya", false, 4, $editedheader1);
    ?>

    <?php
    $from = UI::createTextBox('target_penyelesaian', $rowheader1['target_penyelesaian'], '', '', $editedheader1, $class = 'form-control datepicker');
    echo UI::createFormGroup($from, $rules["target_penyelesaian"], "target_penyelesaian", "Target Penyelesaian", false, 4, $editedheader1);

    ?>

    <?php
    $from = UI::createTextArea('deskripsi', $rowheader1['deskripsi'], '', '', $editedheader1, $class = 'form-control', "");
    echo UI::createFormGroup($from, $rules["deskripsi"], "deskripsi", "Keterangan", false, 4, $editedheader1);
    ?>
  </div>
</div>
<hr />
