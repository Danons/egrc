...
class _Model extends CI_Model
{
	...
	public function GOne($field = "*", $addsql = "")
	{
		...
		$sql = "select {$field} from " . $this->table . " {$addsql} {$ada_sql}  deleted_date is null";
	...
	public function GetByPk($id)
	{
		...
		$sql = "select * from " . $this->table . " where {$this->pk} = " . $this->conn->qstr($id) . " and deleted_date is null";
	...
	function MaxRow()
	{
		$sql = "select * from " . $this->table . " where deleted_date is null order by {$this->pk} desc limit 1";
	...
	function SqlComboDK()
	{
		return "select a.*, {$this->pk} as idkey, {$this->kode} as kode, {$this->label} as val from {$this->table} a where deleted_date is null order by idkey";
		// return "select a.*, {$this->pk} as idkey, {$this->kode} as kode, {$this->label} as val from {$this->table} a order by idkey";
	}

	function SqlCombo()
	{
		return "select {$this->pk} as idkey, {$this->label} as val from {$this->table} where deleted_date is null order by idkey";
	...
	function GetComboLost($id = null, &$arr_combo = array(), $attarr = array())
	{
		...
		$arr_combo[$id] = $this->conn->GetOne("select {$label} from {$table} where deleted_date is null and {$pk} = " . $this->conn->escape($id));
