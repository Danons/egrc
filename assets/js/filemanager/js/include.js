$(document).ready(function(){	    
	...
	$('.new-folder').click(function(){
		...
		if(folder_name){
			folder_path=$('#cur_path').val()+ folder_name;
			$.ajax({
				  type: "POST",
	...
	$('.send-btn').click(function(){
	    ...
	    var thumb = $('#cur_thumb').val();
	    var base_url = $('#base_url').val();
		var closed = window_parent.document.getElementsByClassName('mce-filemanager');
	    var checkedValues = $('input.check_data:checked').map(function() { return this.value; }).get();

	    window.parent.goApplyFile(base_url, path, thumb, checkedValues);
...
function apply_img(file,type_file,external){
    ...
    if (external=="") {
		...
		var targetthumb = window_parent.document.getElementById('thumb_'+track);
		var targetfiles = window_parent.document.getElementById('file_'+track);

		console.log(targetfiles);
		
		if(targetthumb){
			$(targetthumb).attr("src",base_url+thumb+file);
			$(targetthumb).show();
		}

		if(targetfiles){
			$(targetfiles).attr("src",base_url+path+file);
			$(targetfiles).show();
		}
